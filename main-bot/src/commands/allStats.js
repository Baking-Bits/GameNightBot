...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('allstats')
        .setDescription('Show server-wide voice activity statistics'),
    async execute(interaction, bot) {
        await interaction.deferReply();
        
        try {
            console.log('[ALLSTATS] Starting server activity schedule fetch for guild:', interaction.guildId);
            const schedule = await bot.db.getServerActivitySchedule(interaction.guildId);
            console.log('[ALLSTATS] Schedule data received:', {
                hourlyData: schedule.hourlyData?.length || 0,
                dailyData: schedule.dailyData?.length || 0,
                totalUsers: schedule.totalUsers,
            ...
            console.error('[ALLSTATS] Error executing command:', error);
            try {
                ...
                console.error('[ALLSTATS] Error sending error message:', followUpError);
            }
        }
    }
};
