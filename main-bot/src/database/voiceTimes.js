...
async function getServerActivitySchedule(guildId) {
    ...
    try {
        ...
        console.log('[ALLSTATS DEBUG] dailyUsers sample:', dailyUsers.slice(0, 2));
        console.log('[ALLSTATS DEBUG] hourResults sample:', hourResults.slice(0, 2));
        console.log('[ALLSTATS DEBUG] dayResults sample:', dayResults.slice(0, 2));

        const avgDailyUsers = Math.round(dailyUsers.reduce((sum, day) => sum + Number(day.users || 0), 0) / dailyUsers.length) || 0;
        const peakHour = hourResults.reduce((max, curr) =>
            Number(curr.total_time || 0) > Number(max.total_time || 0) ? curr : max,
            { total_time: 0 }
        );
        const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
