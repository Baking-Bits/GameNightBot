    ...
    async awardShittyWeatherPoints() {
        try {
            ...
            if (worstWeather.score > 0) {
                ...
                // Award point to worst weather user
                if (!data.shittyWeatherScores[worstWeather.userId]) {
                    data.shittyWeatherScores[worstWeather.userId] = 0;
                }
                data.shittyWeatherScores[worstWeather.userId]++;

                // Record the award
                const award = {
                    timestamp: new Date().toISOString(),
                    userId: worstWeather.userId,
                    displayName: worstWeather.userData.displayName,
                    region: worstWeather.userData.region,
